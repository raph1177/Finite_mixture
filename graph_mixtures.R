...
#### Graph for the rest of the article ####
CEM_prop_graph = fread(file="C:\\Users\\raph1\\Documents\\Cours\\H2022\\ECN 770\\These\\CEM_prop_graph.txt",dec=",")
CEM_ceti_graph = fread(file="C:\\Users\\raph1\\Documents\\Cours\\H2022\\ECN 770\\These\\CEM_CETI_graph.txt",dec=",")
CEM_ceti_graph = CEM_ceti_graph[,.(`Number of groups`=as.character(Group), Algorithm=Algo, `Cross-validated RMSE`=Value)]
lin_4_data = fread(file="C:\\Users\\raph1\\Documents\\Cours\\H2022\\ECN 770\\These\\lin_4_CEM.txt",dec=",")
lin_5_data = fread(file="C:\\Users\\raph1\\Documents\\Cours\\H2022\\ECN 770\\These\\lin_5_EM.txt",dec=",")


## Boxplot CETI ###
CEM_ceti_graph$`Relative RMSE` =  CEM_ceti_graph$`Cross-validated RMSE`/2.05
CEM_ceti_graph$X_1 = as.character(seq(0,143,1)/100)
CEM_ceti_graph = CEM_ceti_graph[Algorithm=="cCEM", Algorithm:="C-EM"]

p = ggplot(CEM_ceti_graph, aes(x=`Number of groups`, y=`Relative RMSE`, fill=Algorithm)) +
  geom_boxplot()+
  #scale_fill_manual(values=c('#a0a09c', '#5f5f94'))+
  theme_bw()
p
ggsave("fig7_boxplot.pdf", p, height = 5, width = 9, dpi = 300)


## Stacked + percent ##
p = ggplot(data=CEM_prop_graph, aes(fill=Group, y=Value, x=Period)) + 
  geom_bar(position="fill", stat="identity")+
  labs(y = "Proportion of observations", x = "Period")+
  scale_fill_manual(values=c('#e8e8dd', '#f4f4c3', '#a0a09c', '#555573', '#5f5f94'))+
  theme(text = element_text(size = 16))+
  theme_bw()
p
ggsave("fig9_proportions.pdf", p, height = 3.5, width = 7, dpi = 300)

## Fixed effects estimates ##
p = ggplot(data=lin_5_data, aes(x=Variable, y=Coef)) +
  geom_line( linewidth=1, alpha=1, color="steelblue", aes(x=Variable, y=Coef, group=Group))+
  geom_line( linetype="dashed", linewidth=0.1, alpha=0.1, aes(x=Variable, y=Lim_inf, group=Group))+
  geom_line( linetype="dashed", linewidth=0.1, alpha=0.1, aes(x=Variable, y=Lim_sup, group=Group))+
  geom_line( linewidth=0.5, alpha=1, color="black", aes(x=Variable, y=0))+
  geom_ribbon(data=lin_5_data, aes(ymin=Lim_inf,ymax=Lim_sup), fill="grey", alpha=0.65) +
  facet_wrap(~Group, ncol=2, scales = "free")+
  labs(y = "Fixed effect estimates", x = "Period") +
  theme(strip.background =element_rect(fill="azure3"))+
  theme(strip.text = element_text(colour = 'black'))+ 
  theme_bw()
p
ggsave("figf1_fixed_eff.pdf", p, height = 8, width = 7, dpi = 200)

p = ggplot(data=lin_4_data, aes(x=Variable, y=Coef)) +
  geom_line( linewidth=1, alpha=1, color="steelblue", aes(x=Variable, y=Coef, group=Group))+
  geom_line( linetype="dashed", color="grey", linewidth=0.1, alpha=0.1,  aes(x=Variable, y=Lim_inf, group=Group))+
  geom_line( linetype="dashed", color="grey", linewidth=0.1, alpha=0.1, aes(x=Variable, y=Lim_sup, group=Group))+
  geom_line( linewidth=0.5, alpha=1, color="black", aes(x=Variable, y=0))+
  geom_ribbon(data=lin_4_data, aes(ymin=Lim_inf,ymax=Lim_sup), fill="grey", alpha=0.65) +
  facet_wrap(~Group, ncol=2, scales = "free")+
  labs(y = "Fixed effect estimates", x = "Period") +
  theme(strip.background =element_rect(fill="azure3"))+
  theme(strip.text = element_text(colour = 'black'))+
  theme_bw()
p
ggsave("fig8_fixed_eff.pdf", p, height = 8, width = 7, dpi = 200)

### Transition matrix ##
tr_mat = fread(file="C:\\Users\\raph1\\Documents\\Cours\\H2022\\ECN 770\\These\\transition_mat1.txt",dec=",")
tr_mat$V2=as.character(tr_mat$V2)
p = ggplot(tr_mat, aes(V2, V1)) +
  geom_tile(aes(fill = V3)) +
  geom_text(aes(label = round(V3, 2))) +
  scale_fill_gradient(low = "white", high = "#CC3F0c")+
  labs(y = "Group at period t", x = "Group at period t+1")+
  scale_y_discrete(
    limits = rev
  )+
  scale_x_discrete(
...
p 
ggsave("fig10_trans.pdf", p, height = 2.5, width = 6.5, dpi = 200)
